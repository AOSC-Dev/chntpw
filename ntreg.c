...
void parse_db(struct hive *hdesc, int vofs, int blen)
{
  ...
  printf("%04lx   number of parts    = %d\n", D_OFFS(no_part), key->no_part  );

  printf("%04lx   Data list at offset: 0x%0x\n", 
	   D_OFFS(ofs_data),
	   key->ofs_data + 0x1000);
...
int alloc_val_data(struct hive *hdesc, int vofs, char *path, int size,int exact)
{
  ...
  if (size > 4) {
	...
	*ptr = blockofs - 0x1000;
    ...
    datablk = vkofs + (int32_t)((void *)&(vkkey->ofs_data) - (void *)vkkey);
    size |= 0x80000000;
  }
